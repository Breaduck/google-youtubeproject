...
const App: React.FC = () => {
  ...
  const [batchRange, setBatchRange] = useState<{ mode: 'all' | 'missing' | 'custom'; start: number; end: number; customText: string }>({ mode: 'all', start: 1, end: 1, customText: '' });
  ...
  const exportVideo = async (sampleIndices?: number[], sampleDuration?: 30 | 60) => {
    if (!project) return;

    // 예시 영상 모드인지 확인
    const isSampleMode = sampleIndices && sampleIndices.length > 0;
    const scenesToExport = isSampleMode
      ? sampleIndices.map(idx => project.scenes[idx - 1]).filter(Boolean)
      : project.scenes;

    // 이미지와 오디오 확인
    const missingAssets = scenesToExport.some(s => !s.imageUrl || !s.audioUrl);
    if (missingAssets) {
      alert(isSampleMode ? "선택한 씬의 이미지와 오디오가 모두 생성되어야 합니다." : "모든 장면의 이미지와 오디오가 생성되어야 합니다.");
      return;
    }
    ...
    const exportLabel = isSampleMode ? `${sampleDuration}초 예시` : '';
    setBgTask({ type: 'video', message: `${exportLabel ? exportLabel + ' ' : ''}${engineName} 비디오 생성 중...` });
    setBgProgress(0);
    ...
    try {
      ...
      for (let i = 0; i < totalScenes; i++) {
        ...
        const originalIdx = isSampleMode ? (sampleIndices![i]) : (i + 1);
        console.log(`[DEBUG] Scene ${originalIdx}/${project.scenes.length} - Starting ${engineName} generation`);
        ...
        if (hasExistingVideo && scene.videoUrl) {
          ...
          videoBlob = await generateSceneVideo(
            ...
            (progress, message) => {
              const baseProgress = Math.round((i / totalScenes) * 50);
              const sceneProgress = Math.round((progress / 100) * (50 / totalScenes));
              setBgProgress(baseProgress + sceneProgress);
              setBgTask({ type: 'video', message: `${exportLabel ? exportLabel + ' ' : ''}${message} (${i + 1}/${totalScenes})` });
            }
          );
        } else {
          // 범위 밖 or API 키 없음 or 예시 모드 → 맥락 기반 줌인/줌아웃
        ...
        let finalVideoBlob = videoBlob;
        if (scene.audioUrl) {
          setBgTask({ type: 'video', message: `${exportLabel ? exportLabel + ' ' : ''}오디오 통합 중 (${i + 1}/${totalScenes})...` });
          finalVideoBlob = await addAudioToVideo(
            videoBlob,
            scene.audioUrl,
            (progress, message) => {
              const baseProgress = Math.round((i / totalScenes) * 50);
              const audioProgress = Math.round((progress / 100) * (10 / totalScenes));
      ...
      // 2단계: 비디오 병합 및 다운로드
      setBgTask({ type: 'video', message: `${exportLabel ? exportLabel + ' ' : ''}비디오 병합 중...` });
      ...
      if (videoBlobs.length === 1) {
        ...
        finalBlob = await mergeVideos(videoBlobs, (progress, message) => {
          ...
          setBgTask({ type: 'video', message: `${exportLabel ? exportLabel + ' ' : ''}${message}` });
        });
      }

      // 통합 비디오 다운로드
      setBgProgress(100);
      setBgTask({ type: 'video', message: `${exportLabel ? exportLabel + ' ' : ''}비디오 다운로드 중...` });
      const url = URL.createObjectURL(finalBlob);
      const a = document.createElement('a');
      a.href = url;
      a.download = isSampleMode ? `${project.title}_sample_${sampleDuration}s.mp4` : `${project.title}_complete.mp4`;
      a.click();
      URL.revokeObjectURL(url);

      await new Promise(resolve => setTimeout(resolve, 500));
      alert(isSampleMode ? `${sampleDuration}초 예시 영상 다운로드 완료!` : `통합 비디오 다운로드 완료!`);
      setBgTask(null);
      setBgProgress(0);
    } catch (error: any) {
  ...
  return (
    <div className={`min-h-screen bg-[#FDFDFD] dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans selection:bg-indigo-100 dark:selection:bg-indigo-900 pb-40 transition-colors duration-500`}>
                      ...
                      <div className="relative group/preview" onMouseEnter={() => setIsPreviewHovered(true)} onMouseLeave={() => setIsPreviewHovered(false)}>
                        ...
                        {project.scenes.some(s => !s.imageUrl || !s.audioUrl) && (() => {
                          ...
                          if (totalMissing >= 2) {
                            ...
                            if (imageCount > 0 && audioCount > 0) message = '일부 씬의 이미지와 오디오가 없습니다';
                            else if (imageCount > 0) message = '일부 씬의 이미지가 없습니다';
                            else message = '일부 씬의 오디오가 없습니다';
                          } else {
              ...
              <div className="mb-4 flex justify-end gap-3">
                ...
                <div className="relative group/download">
                  ...
                  <div className="absolute top-full right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-xl py-2 min-w-[200px] opacity-0 invisible group-hover/download:opacity-100 group-hover/download:visible transition-all z-50">
                    ...
                    <button onClick={() => setShowGlobalExportModal(true)} className="w-full px-4 py-2.5 text-left text-sm text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-all font-medium whitespace-nowrap">🌍 글로벌 버전으로 내보내기</button>
                  </div>
                </div>
              </div>
                      ...
                      <div className="absolute top-3 right-3 z-30 flex gap-1.5 opacity-0 group-hover/card:opacity-100 transition-all">
                        ...
                        {scene.uploadedVideoUrl && (
                          <div className="relative group/media">
                            ...
                            <div className="absolute top-full right-0 mt-1 bg-white dark:bg-slate-800 rounded-lg shadow-xl py-1 min-w-[100px] opacity-0 invisible group-hover/media:opacity-100 group-hover/media:visible transition-all z-50">
                              <button onClick={(e) => { e.stopPropagation(); updateCurrentProject({ scenes: project.scenes.map(s => s.id === scene.id ? { ...s, activeMedia: 'image' } : s) }); }} className={`w-full px-3 py-2 text-left text-xs transition-all flex items-center gap-2 ${scene.activeMedia === 'image' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
                                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                이미지
                              </button>
                              <button onClick={(e) => { e.stopPropagation(); updateCurrentProject({ scenes: project.scenes.map(s => s.id === scene.id ? { ...s, activeMedia: 'video' } : s) }); }} className={`w-full px-3 py-2 text-left text-xs transition-all flex items-center gap-2 ${scene.activeMedia === 'video' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
                        ...
                        {scene.imageUrl || scene.uploadedVideoUrl ? (
                          <div className="relative group/delete">
                            ...
                            <div className="absolute top-full right-0 mt-1 bg-white dark:bg-slate-800 rounded-lg shadow-xl p-1.5 opacity-0 invisible group-hover/delete:opacity-100 group-hover/delete:visible transition-all z-50">
                              <div className="flex gap-1 mb-1">
                                {scene.imageUrl && <button onClick={() => deleteSceneImage(scene.id)} className="px-2.5 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded transition-all whitespace-nowrap">이미지 삭제</button>}
                                {scene.uploadedVideoUrl && <button onClick={() => { if(scene.uploadedVideoUrl?.startsWith('blob:')) URL.revokeObjectURL(scene.uploadedVideoUrl); updateCurrentProject({ scenes: project.scenes.map(s => s.id === scene.id ? { ...s, uploadedVideoUrl: null, activeMedia: 'image' } : s) }); }} className="px-2.5 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded transition-all whitespace-nowrap">영상 삭제</button>}
                              </div>
                              <button onClick={() => deleteScene(scene.id)} className="w-full px-2.5 py-1.5 text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-all">씬 삭제</button>
                            </div>
                          </div>
                        ) : (
                          ...
                          <div className="absolute inset-0 bg-black/30 opacity-0 group-hover/img:opacity-100 transition-all flex items-center justify-center gap-3 z-10 cursor-pointer" onClick={() => setSelectedImage(scene.imageUrl)}>
                            ...
                            <div className="relative group/dl">
                              ...
                              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-white dark:bg-slate-800 rounded-xl shadow-xl p-1.5 opacity-0 invisible group-hover/dl:opacity-100 group-hover/dl:visible transition-all z-50 flex gap-1">
                                <button onClick={(e) => { e.stopPropagation(); if(scene.imageUrl) { const a = document.createElement('a'); a.href = scene.imageUrl; a.download = `scene-${idx+1}.png`; a.click(); }}} className="px-2.5 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded transition-all whitespace-nowrap">이미지만</button>
                                <button onClick={async (e) => { e.stopPropagation(); if(scene.imageUrl && scene.scriptSegment) { setLoadingText('줌인-줌아웃 영상 생성 중...'); try { const { generateSimpleZoomVideo } = await import('./services/videoService'); const zoomSettings = determineZoomSettings(scene.scriptSegment, idx); const videoBlob = await generateSimpleZoomVideo(scene.imageUrl, scene.scriptSegment, zoomSettings.direction, subtitleSettings, undefined, zoomSettings.pan, zoomSettings.intensity); const url = URL.createObjectURL(videoBlob); const a = document.createElement('a'); a.href = url; a.download = `scene-${idx+1}_zoom.mp4`; a.click(); URL.revokeObjectURL(url); } catch(err) { alert('영상 생성 실패'); } setLoadingText(''); }}} className="px-2.5 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded transition-all whitespace-nowrap">자막 포함 영상</button>
                              </div>
                            </div>
                          </div>
                          )}
      ...
      {batchModal.open && project && (() => {
        ...
        const targetScenes = batchRange.mode === 'all' ? scenes : batchRange.mode === 'missing' ? scenes.filter((_, i) => missingIdx.includes(i + 1)) : scenes.filter((_, i) => customIndices.includes(i + 1));
              ...
              <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-2">
                ...
                <button
                  onClick={() => {
                    ...
                    const indices = batchRange.mode === 'all' ? null : batchRange.mode === 'missing' ? missingIdx : customIndices;
                    if (type === 'image') generateAllImages(indices);
                    else if (type === 'audio') generateBatchAudio(indices);
                    else openVideoGenerationPopup(indices);
                  }}
